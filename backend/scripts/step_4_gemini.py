...
if not GEMINI_API_KEYS:
    raise ValueError("Error: GEMINI_API_KEYS is not set in the .env file.")

# Split the API keys into a list and create a cycle iterator
...
model = configure_gemini_api(current_api_key)
...
def refine_chunk_with_gemini(chunk, api_key_cycle):
    ...
    while True:
        try:
            ...
            print(f"Error refining chunk with API key {current_api_key}: {e}")
            # Switch to the next API key
            current_api_key = next(api_key_cycle)
            print(f"Switching to next API key: {current_api_key}")
            # Reconfigure the Gemini API with the new API key
            model = configure_gemini_api(current_api_key)
...
def refine_chapters(chapter_contents, api_key_cycle):
    refined_chapters = []
    logs = []
    ...
    with tqdm(total=total_chunks, desc="Refining Chapters", unit="chunk") as pbar:
        for chapter_number, heading, chapter_content in chapter_contents:
            ...
            for chunk in chunks:
                response_data = refine_chunk_with_gemini(chunk, api_key_cycle)
                refined_chunks.append(response_data["raw_response"] if response_data["raw_response"] else chunk)
...
if __name__ == "__main__":
    ...
    refined_chapters, logs = refine_chapters(chapter_contents, api_key_cycle)
