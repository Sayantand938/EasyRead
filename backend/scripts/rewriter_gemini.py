    ...
    while True:  # Keep retrying until a successful response is received
        try:
            ...
            response = model.generate_content(
                f"You are a expert text refinement assistant who refines text taken from popular books. Replace the advanced vocabulary with simpler alternatives. Preserve the original structure like headings and other things. Stick to the original source as much as possible while simplifying complex sentences into easier to understand sentences. Point to note this paragraphs are taken from popular books so it has literary values try to respect that. Just reply with the refined text.:\n\n{chunk_text}"
            )
